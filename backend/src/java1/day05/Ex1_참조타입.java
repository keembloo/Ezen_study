...
public class Ex1_참조타입 {
	...
	// 스택구역 = 힙구역
		...
		arr3 = arr2;	// arr2 스택변수가 가지고있던[101번지]를 arr3 스택변수에 대입
		...
		hobby = null;
			...
			// * hobby = null
			// 5. 아무도 힙영역에 있는 "여행" 문자열 객체를 참조하지 않으면 GC 쓰레기수집기 자동으로 객체 삭제
...
/*
	...
	자바 데이터 타입 : 변수 선언할때 사용되는 타입
		- 기본타입 : boolean byte short char int long float double
			- 변수에 값 자체 저장
			- 8가지 존재 / 소문자 시작
		- 참조타입 : 배열 , 열거 , 클래스[String , Scanner , Random ] , 인터페이스
	...
	JVM 메모리		
		...
		스택영역							힙영역
		- 함수(프레임)마다 별도구역			- 객체가 생성되는 영억
			-지역변수							- 스택/메소드 영역에 참조 할수 있다.
			- 함수 호출시 프레임 생성			- 객체, 배열, 인터페이스 메모리 생성
			-함수(프레임) 종료되면 제거			- new 연산자 : 타입/클래스 기반으로 메모리생성후 주소/번지 반환
