...
public class Ex1_배열 { // class s
	public static void main(String[] args) { // main s
		...
		// 5. 추가 ( 없음 : 배열은 선언될때 고정!! ) * 배열내 인덱스중에 null 있으면 해당위치에 데이터 대입
		...
		// 총합 / 평균
		int sum = 0;
		for (int i = 0; i<3 ; i++) { // i는 0부터 3미만까지 1씩증가 [ 0~2 ] // scores 배열은 [0~2]
			sum += scores[i];
		...
		//2. double형 배열
				//1. new 연산자를 이용한 배열 선언
				double[] arr2 = new double[3];  // double 3개를 젖아할수 있는 배열 선언 [ 0.0 , 0.0 , 0.0 ]
				...
				// arr2.push() [x] : JS에서 사용되는 함수이므로 자바에서 불가능
		...
		// 3. String 클래스 배열
				...
				System.out.println(Arrays.toString(arr3));
					...
					for (int i=1; i<arr3.length; i++) {
						// 만약에 마지막인덱스에 도착했으면 다음인덱스가 없으므로 당기기 할 필요 x
...
/*
 ...
 배열 : 여러개의 데이터를 저장
 	...
 	3. 특징
 		1. 배열은 선언된 타입에 값만 저장할수 있다.
 			...
 			* int로 선언된 배열은 int형 데이터 저장가능
 			int[] intArray = { 1, 3 } [o]
 		2. 배열의 길이는 선언될때 고정되므로 늘리거나 줄일수 없다.
	...
	4. 선언
		...
		2. 선언과 동시에 값 대입
			타입[] 변수명 = { 값1 , 값2 , 값3 , 값4 }
		3. new 연산자로 배열 생성 초기값(기본값)으로 생성
			타입[] 변수명 = new 타입[길이];		*길이 : 배ㅔ열에 저장할 데이터 개수
