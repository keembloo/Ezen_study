...
/*		
		...
		// 힙영역 : 객체/배열은 참조하는 스택이 없으면 자동으로 메모리가 초기화
			// ---> RAM 주기억장치	 : 현재 처리된 명령어 저장 --> 전기x 저장x -> 휘발성 
		// 영구저장
		...
		// 파일처리 라이브러리( 미리 만들어진 클래스/메소드 )
			...
			// 1. FileOutputStream : 파일 내보내기/저장 관련된 메소드 제공하는 클래스
				// 1.  new FileOutputStream("파일경로");	:	해당파일과 연동/연결
					// 만약에 파일경로에 해당 파일이 존재하지 않으면 해당 파일 생성
		
				// new FileOutputStream("파일경로" , true );	:	해당파일과 연동/연결
					// 만약에 파일경로에 해당 파일이 존재하지 않으면 해당 파일 생성 , 존재하면 이어쓰기
				// 파일경로 :
					// 상대경로 : 프로젝트명생략 -> ./src/패키지명/파일명
					// 절대경로 : C:\Users\504\git\Ezen_study\backend\src\java1\day07
		
			// 2. FileInputStream : 파일불러오기 관련된 메소드 제공하는 클래스
				//new FileInputStream("파일경로")		: 해당파일과 연동/연결
	
		
			// 2. 제공하는 함수/메소드
				// 1. .write( 바이트배열 )				: 해당 객체와 연결된 파일의 바이트 내보내기
		        // 2. .read( 바이트배열 )				: 해당 객체와 연결된 파일의 바이트 읽어와서 바이트 배열에 저장
		
			// 1. 문자열.getBytes()				: 해당 문자열을 바이트배열 반환 
			// 2. new String( 바이트배열 ); 		: 해당 바이트배열을 문자열로 반환
		
		
			// 3. File				: 파일에 관련 메소드 제공
			 	new File("파일경로")
				메소드
					1. .exists()	: 파일 경로에 파일 존재여부 true / false 반환
					2. .delete()	: 파일 경로에 파일 삭제 실행
					3. .getPath()	: 파일이 위치한 경로 문자열 반환
					4. .length()	: 파일의 용량 (바이트)용량을 long타입 반환
...
*/		
		...
		FileOutputStream fieFileOutputStream = new FileOutputStream("./src/java1/day07/Ex1_테스트.txt" , true); // 이어쓰기 기능추가
		...
		File file = new File("./src/java1/day07/로또.scv");
