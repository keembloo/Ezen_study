...
public class Board {
	...
	*/
	...
	// 1. 필드	: 객체의 데이터 저장하는 곳
		/*
		 - 필드 종류
		 	- 인스턴스필드		: (힙영역) 객체 생성시 객체 안에 저장되어있는 필드 -사용
		 	- 정적필드		: (메소드영역) [static] 프로그램 실행시 우선할당되고 객체없이 저장되어있는 필드 -사용
		 
		 - 필드 타입
		 	- 기본타입		: boolean char byte short int long float double 8개
		 ...
		 -싱글톤				: 프로그램내 1개만 존재하는 객체(여러개 생성 불가능)
	...
	// 2. 생성자	: 객체 초기화/처음값대입 할때 사용
		/*
			...
			- 생성자 종류 [ 오버로딩으로 인해 다양하게 선언 가능 ]
				- 오버로딩 : 매개변수의 개수,순서,타입에따라서 식별 가능한 것
			
			- 접근제한 private , public , default
			
			- 기타 키워드
				- this 		: (생략가능) 현재 클래스의 구성멤버 식별할때 사용
				- this.필드명	: 외부로부터 들어온 매개변수명과 현재클래스내 필드명과 같을때 필드 식별할때
				- this()	: 현재 클래스의 생성자 호출
	...
	/*
		...
		- 메소드 선언
			접근제한자 리턴타입 메소드명( 타입 매개변수, 타입 매개변수 ){ }
				...
				- { }		: 메소드 실행코드
				- return 	: (void 생략가능) 리턴타입에 맞는 값을 리턴후 메소드 종료
		...
		- MVC 디자인 패턴 : 기능역할에 따른 구분
			...
			M : 데이터 모델과 실제 데이터
				DTO : 데이터 이동 객체 [ 서로 다른 레이어간 이동할때 객체사용/ 포장지 ]
					...
					2. DTO 가독성
						member		: 회원객체인데 무슨객체?
						memberDto	: 회원객체인데 이동객체구나 파악할 수 있다
