...
public class BoardDto {
	// 싱글톤X : 게시물1개당 DTO 1개이기때문에 여러 DTO 필요
	...
	public void setView(int view) {
		...
		// 2. (권장) toString()		  : 객체호출시 주소값이 반환되지만 객체내 필드값을 반환하고싶을때 사용되는 메소드
	@Override
...
/*
	DTO : 이동객체
		...
		2. 생성자
			...
			3. 그외 사용에따라 선언
		3. 메소드
			1. setter and getter	: 필드가 직접적인[private] 접근 불가능하기 때문에 메소드를 이요한 필드 간접접근
				set필드명()	: 필드에 값 대입할때 사용되는 메소드
				get필드명()	: 필드에 값 호출할때 사용되는 메소드
			2. toString()		: 객체호출시 주소값이 반환되지만 객체내 필드값을 반환하고싶을때 사용되는 메소드
