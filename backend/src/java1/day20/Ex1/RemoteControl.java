...
public interface RemoteControl { // 인터페이스 선언
	...
	// 구성멤버
		// 1. public 상수 필드			[ public static final 생략가능 ]
		int MAX_VOLUME = 10;
		public static final int MIN_VOLUEM = 0;
	
		// 2. *public 추상 메소드			[ public abstract 생략가능 ] 	: 선언부	=> 구현객체 O
		void turnOn();
		...
		default void setMute(boolean mute) {
			if (mute) {System.out.println("무음처리합니다"); setVolume(MIN_VOLUEM);}
			else {System.out.println("무음 해제합니다");}
		}
		public default void getMute() {System.out.println("");}
		
		// 4. public 정적(객체없이) 메소드	 [ public 생략가능 ] static 	: 선언부 + 실행부	=> 구현객체 X
...
/*
	...
	정잭멤버 	  : 객체가 없어도 메모리 우선할당된 필드,메소드	[static 있는]
