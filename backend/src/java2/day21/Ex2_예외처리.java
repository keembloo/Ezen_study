...
public class Ex2_예외처리 {
	public static void main(String[] args) {
		// p.470 : 다중 catch : 다양한 예외에 다른 서로 다른 흐름제어
		...
		for ( int i =0; i<=array.length; i++) { // 인덱스 : 0~1 // 길이 : 2 // 인덱스 2는 없음
			...
			// catch (Exception e) {System.out.println("예외발생 : "+e);} // 예외 상위 클래스가 앞에 있으면 아래있는 하위 클래스들은 실행 안됨
			catch (NumberFormatException e) {
				System.out.println("숫자로 변환 할수없는 데이터 : "+e);
			} catch (ArrayIndexOutOfBoundsException e) {
				System.out.println("배열 인덱스 초과 : "+e);
			} catch (NullPointerException | ClassCastException e) {
				System.out.println("데이터에 문제가있음 : "+e);
