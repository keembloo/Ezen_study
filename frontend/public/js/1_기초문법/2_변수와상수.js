/*
	...
	- 변수 : 데이터 1개 저장하는 메모리공간
		- 저장된 데이터는 변경이 가능하다.
		- let , var 변수 선언시 사용되는 키워드 [ var 키워드 : 변수명 중복가능 (사용권장x)]
		- 변하는 값 , [ 임시 저장 ]
		
	- 상수 : 데이터 1개 저장하는 메모리공간
		- 저장된 데이터는 변경이 불가능하다.
		- const 상수 선언시 사용되는 키워드
		- 고정된값 , 변하지 않는 값, 일정한 값 [파이(3.14) 등등]
		
	- 변수/상수 선언/만들기 형태
		1. 선언 키워드 [ let vs const ]
		2. 변수/상수 이름 [ 영문, 띄어쓰기 X, 숫자시작X, 카멜표기법 ]
...
// 4. 변수의 연산
let 변수2 = 변수1+10 // 변수2 = 20+10 // 변수명 호출시 변수안에 있는 데이터가 출력
...
let 상태 = confirm('삭제하시겠습니까?')	// confirm() : 확인(true) or 취소(false) 버튼 존재하는 메세지 함수
