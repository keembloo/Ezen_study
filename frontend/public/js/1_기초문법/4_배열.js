	...
	배열 : 데이터 여러개를 한곳에 저장할수 있는 공간/메모리/상자		
		...
		7. 배열내 데이터 추가
			...
			배열명.splice(추가할위치인덱스 , 0 , 새로운값)	:	해당 위치인덱스에 새로운 값 저장
				- 특정 위치/인덱스에 데이터 추가
				[ * 추가된 요소 뒤로 한칸씩 자동 당겨짐 ]
				
		8. 배열내 데이터 삭제
			배열명.pop()		: 배열내 마지막 위치 데이터 삭제
			배열명.splice( 제거할 인덱스 , 제거개수 , [새로운값])
			ex)
		...
		9. 배열내 데이터를 이용한 데이터 삭제
				- 배열명.indexOf('찾을데이터')	:	찬을 데이터가 존재하면 찾은 인덱스번호 아니면 -1
...
document.write(배열명)
// 예제 3 : 인덱스 사용 ( 수정 or 삭제할때)
console.log(배열명[0])
document.write('<h3> 과일개수 : '+ 배열명.length + '</h3>')	// html 문자처리o
...
// 예제 5 : 배열에 요소/데이터 추가
배열명.push('체리');	// push('체리') : 배열내 마지막 요소/데이터 뒤에 '체리' 데이터 추가
console.log(배열명);
...
console.log(배열명);  //복숭아제거
배열명.splice( 배열명.indexOf('딸기'), 1) // '딸기' 삭제하는데 인덱스번호를 모르는 상태
	// 1. 배열명.indexOf('딸기') => 2
