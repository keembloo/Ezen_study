/*
	...
	- 제어문 = if = 만약에
		...
		if(A편의점_콜라 <= 1000) {
			구매하고 집으로 간다
		} else if ( B편의점_콜라 <=1000) {
			구매하고 집으로 간다
		} else if ( C편의점_콜라 <=1000) {
			구매하고 집으로 간다
		} else { 그냥 집간다 };
	...
	2. if[*] , switch
	4. if 형태
		* 조건 : true=1 or false=0 결과를 갖는 변수 혹은 연산/계산
		* 조건과 참/거짓의 코드가 이어져야한다. [if(조건);참일경우코드]
		1. if(조건) 참일경우 코드
		2. if(조건) {참일경우 코드1; 참일경우 코드2;}
		3. if(조건) {참일경우코드}
			else {거짓일경우코드}
					-------vs------(삼항연산자) 조건 ? 참 : 거짓
		4. if(조건) {참1일경우 코드}
...
if(10>20); console.log('참[3] 10이 20보다 크다.') // 중간에 ; 있으면 안된다.
