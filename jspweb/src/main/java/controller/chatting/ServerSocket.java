	...
	public void onClose (Session session) {
		...
		for (ClientDto clientDto : clientList) {
			...
			if (clientDto.getSession() == session ) {
				// 클라이언트소켓의 세션과 연결이 끊긴 세션과 같으면 해당 dto 제거하기
...
/*
	...
	* WebSocket 서버소켓 라이브러리 제공
	 	...
	 	2. 서버소켓 메소드 제공
			...
			2. 클라이언트 소켓과 연결이 끊겼을때 (자동실행)되는 메소드
			@OnClose
				public void onClose (Session session) {}
					- Session : 서버소켓과 연결이 끊긴 클라이언트소켓 정보
					
			3. 클라이언트 소켓과 오류가 발생했을때 (자동실행)되는 메소드	
			@OnError
				public void onError (Session session , Throwable throwable) {}
					- Session : 서버소켓과 오류가 발생한 클라이언트소켓 정보
					- Throwable : 오류 발생한 사유 정보
					
			4. 클라이언트 소켓으로부터 메세지를 받았을때 (자동실행)되는 메소드
			@OnMessage
				public void onMessage( Session session , String msg) {}
					- Session : 서버소켓에게 메세지를 보낸 클라이언트소켓 정보
					- String 매개변수 : 클라이언트소켓으로부터 받은 메세지 [ 문자열타입 ] 
