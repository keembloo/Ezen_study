...
public class EmailService { // 이메일 전송에 관련된 클래스
	...
	// 1. 필드
	private String fromEmail = "asdf@naver.com"; // 보내는사람의 이메일 [ 관리자 이메일 ]
	private String fromEmailPwd = "asdf"; // 보내는 사람의 이메일주소 비밀번호
	...
	public boolean authsend( String toEmail , String contentHTML ) {
		...
		Authenticator authenticator = new Authenticator() {
			...
			// Authenticator : import java.net.Authenticator;
				// - 패스워드 검증[인증] 메소드 구현
				// .getPasswordAuthentication(return new PasswordAuthentication("보내는사람 이메일", "보내는사람의이메일패스워드");) : 
...
/*
	...
	- 자바에서 이메일 전송 보내기
		...
		1. 이메일을 전송할 도메인 필요 [ 네이버, 다음 , 구글 등등 ]
			...
			- 네이버 로그인
				1. 메일
				2. 메뉴 - 환경설정 - POP3/IMAP 설정 - IMAP/SMTP 
				3. IMAP/SMTP 사용 : 사용함
				4. [ 스마트폰 메일 어플리케이션 계정 설정 ] 
					SMTP 서버명 : smtp.naver.com
					SMTP 포트 : 587, 보안 연결(TLS) 필요 (TLS가 없는 경우 SSL로 연결)
		
		2. SMTP 라이브러리 [ 이메일 전송 에 관련된 클래스/라이브러리 제공 ]
		...
		3. 메일 전송 함수 만들기
			...
			4. 보내는 내용 설정
				...
				message.setFrom(new InternetAddress("보내는 사람의 이메일주소"));
